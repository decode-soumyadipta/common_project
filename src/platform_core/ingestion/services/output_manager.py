...
class OutputManager:
    ...
    def get_cog_output_path(
        self, source_path: Path, session_dir: Optional[Path] = None
    ) -> Path:
        """
        ...
        if session is None:
            raise ValueError(
                "No session directory created - call create_session_directory() first"
            )
    ...
    def save_metadata(
        self, source_path: Path, metadata: Dict, session_dir: Optional[Path] = None
    ) -> Path:
        """
