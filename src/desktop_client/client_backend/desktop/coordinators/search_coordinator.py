...
class SearchCoordinator:
    ...
    def search_assets_by_coordinate_event_driven(self) -> None:
        ...
        try:
            ...
            c.panel.set_search_busy(True, "Processing metadata on server...", progress=15)
            ...
            c.panel.set_search_busy(True, "Optimizing results for display...", progress=70)
            ...
            c.panel.set_search_busy(True, "Finalizing event-driven search...", progress=95)
            
            # Track performance
            search_time = time.time() - start_time
            c._track_performance_metric("search_times", search_time, f"coordinate search: {len(assets)} results")
    ...
    def search_assets_from_drawn_geometry_event_driven(self) -> None:
        ...
        c.panel.set_search_busy(True, "Server-side polygon metadata search...", progress=8)
        
        try:
            c.panel.set_search_busy(True, "Processing polygon metadata on server...", progress=20)
            ...
            c.panel.set_search_busy(True, "Server metadata polygon query...", progress=40)
            ...
            c._apply_search_results_event_driven(assets, label=f"Metadata {geometry_type} search")
            
            c.panel.set_search_busy(True, "Finalizing polygon search...", progress=95)
            
            # Track performance
            search_time = time.time() - start_time
            c._track_performance_metric("search_times", search_time, f"polygon search: {len(assets)} results")
    ...
    def _server_metadata_point_search(self, lon: float, lat: float) -> list[dict]:
        ...
        c._logger.info("Server metadata point search completed: %d assets", len(optimized_assets))
        return optimized_assets

    def _server_metadata_polygon_search(self, points: list[tuple[float, float]], buffer_meters: float) -> list[dict]:
        """Perform server-side metadata-only polygon search."""
        c = self._controller
        
        # Use existing API but with metadata-only optimization hints
        assets = c.api.search_assets_by_polygon(points=points, buffer_meters=buffer_meters)
        
        # Server-side metadata optimization: Pre-process for terabyte-scale performance
        optimized_assets = self._optimize_assets_metadata(assets)
        
        c._logger.info("Server metadata polygon search completed: %d assets", len(optimized_assets))
        return optimized_assets
    ...
    def _optimize_assets_metadata(self, assets: list[dict]) -> list[dict]:
        ...
        c._logger.info("Optimized %d assets for terabyte-scale performance", len(optimized))
        return optimized
