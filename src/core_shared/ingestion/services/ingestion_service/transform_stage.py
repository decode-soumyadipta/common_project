...
class TransformToEPSG3857Stage(IngestionStage):
    ...
    def _get_warp_options(self, input_path: Path) -> gdal.WarpOptions:
        ...
        if total_pixels > 100_000_000:  # > 100 megapixels
            LOGGER.info(f"Large raster detected ({total_pixels:,} pixels), using conservative settings")
        ...
        if band_count > 4:
            LOGGER.info(f"Multi-band raster detected ({band_count} bands), optimizing for band processing")
            creation_options.append('INTERLEAVE=BAND')  # Band-interleaved for multi-band
...
class GenerateMBTilesStage(IngestionStage):
    ...
    def run(self, context: IngestionContext) -> None:
        ...
        try:
            ...
            LOGGER.info(f"Input raster analysis: {raster_info['band_count']} bands, "
            ...
            mbtiles_input = self._prepare_mbtiles_input(context.working_path, raster_info)
            ...
            if mbtiles_input != context.working_path and mbtiles_input.exists():
                try:
                    ...
                    LOGGER.warning(f"Failed to clean up intermediate file {mbtiles_input}: {e}")
            ...
            if output_path.exists():
                try:
                    ...
                    pass
            # Clean up intermediate files
            if 'mbtiles_input' in locals() and mbtiles_input != context.working_path and mbtiles_input.exists():
                try:
                    mbtiles_input.unlink()
                except Exception:
                    pass
    ...
    def _analyze_input_raster(self, input_path: Path) -> dict:
        ...
        info = {
            ...
            'width': ds.RasterXSize,
            'height': ds.RasterYSize,
            'data_type': ds.GetRasterBand(1).DataType if ds.RasterCount > 0 else gdal.GDT_Unknown,
            'has_nodata': False,
            'nodata_values': []
        }
    ...
    def _prepare_mbtiles_input(self, input_path: Path, raster_info: dict) -> Path:
        ...
        LOGGER.info(f"Converting {band_count}-band raster to RGB for MBTiles compatibility")
    ...
    def _validate_mbtiles(self, mbtiles_path: Path) -> None:
        ...
        try:
            ...
            if ds is None:
                raise RuntimeError(f"Generated MBTiles file cannot be opened: {mbtiles_path}")
            ...
            ds = None
            LOGGER.info(f"MBTiles validation successful: {mbtiles_path} ({file_size:,} bytes)")
