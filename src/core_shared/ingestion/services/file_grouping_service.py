...
from core_shared.ingestion.services.metadata_extractor import extract_metadata, MetadataExtractorError
...
class FileGroupingService:
    ...
    RASTER_EXTENSIONS = {
        '.tif', '.tiff', '.jp2', '.j2k', '.jpg', '.jpeg', 
        '.png', '.bmp', '.gif', '.hdf', '.nc', '.grib', '.dem'
    }
    ...
    def group_files_in_folder(
        ...
        self._logger.info(f"Categorized: {len(raster_files)} raster files, {len(auxiliary_files)} auxiliary files")
    ...
    def _is_relevant_file(self, file_path: Path) -> bool:
        ...
        if simple_suffix in skip_extensions and full_suffix not in self.AUXILIARY_EXTENSIONS:
            return False
    ...
    def _create_file_groups(
        ...
        if ungrouped_rasters:
            geographic_groups = self._group_by_geography(ungrouped_rasters, auxiliary_files)
            groups.extend(geographic_groups)
        
        # Strategy 3: Directory-based grouping (lower confidence)
        # For remaining files
        remaining_rasters = self._get_ungrouped_rasters(ungrouped_rasters, geographic_groups)
        if remaining_rasters:
            directory_groups = self._group_by_directory(remaining_rasters, auxiliary_files)
    ...
    def _group_by_basename(
        ...
        for raster_file in raster_files:
            ...
            group = FileGroup(
                ...
                scene_name=basename,
                confidence_score=0.9 if matching_aux else 0.7,  # Higher if has aux files
        ...
        for raster_file, metadata in file_metadata.items():
            ...
            if len(related_files) > 1:
                ...
                group = FileGroup(
                    ...
                    bounds=self._merge_bounds([file_metadata[f].bounds for f in related_files]),
    ...
    def _basenames_match(self, basename1: str, basename2: str) -> bool:
        ...
        patterns = [
            r'_processed$', r'_3857$', r'_4326$', r'_utm$', r'_wgs84$',
            r'_reprojected$', r'_clipped$', r'_masked$', r'_final$'
        ]
    ...
    def _bounds_are_related(self, bounds1: Bounds, bounds2: Bounds, threshold: float = 0.1) -> bool:
        """Check if two bounds are spatially related (overlapping or adjacent)."""
        # Calculate overlap
        overlap_x = max(0, min(bounds1.max_x, bounds2.max_x) - max(bounds1.min_x, bounds2.min_x))
        overlap_y = max(0, min(bounds1.max_y, bounds2.max_y) - max(bounds1.min_y, bounds2.min_y))
        ...
        x_adjacent = (
            abs(bounds1.max_x - bounds2.min_x) < distance_threshold or
            abs(bounds2.max_x - bounds1.min_x) < distance_threshold
        )
        y_adjacent = (
            abs(bounds1.max_y - bounds2.min_y) < distance_threshold or
            abs(bounds2.max_y - bounds1.min_y) < distance_threshold
        )
    ...
    def _get_ungrouped_rasters(
        ...
        existing_groups: List[FileGroup]
    ) -> List[Path]:
