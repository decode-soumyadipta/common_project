...
from core_shared.ingestion.services.file_grouping_service import FileGroupingService, FileGroup
...
class FolderIngestionService:
                ...
                try:
                    ...
                    if group_result["success"]:
                        ...
                        if not options.continue_on_error:
                            self._logger.error(f"Stopping ingestion due to error in group: {group.scene_name}")
                            break
                
                except Exception as e:
                    error_msg = f"Unexpected error processing group {group.scene_name}: {e}"
    ...
    def _process_file_group(
        ...
        try:
            self._logger.info(f"Processing file group: {group.scene_name} (method: {group.grouping_method})")
            
            # Check if primary file already exists in catalog
            if options.skip_existing and self._file_exists_in_catalog(group.primary_file, session):
    ...
    def _file_exists_in_catalog(self, file_path: Path, session: Session) -> bool:
        ...
        try:
            ...
            existing = session.query(RasterAsset).filter(
                RasterAsset.file_path == normalized_path
            ).first()
    ...
    def get_folder_statistics(self, folder_path: Path, recursive: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for group in file_groups:
                ...
                method_distribution[group.grouping_method] = method_distribution.get(group.grouping_method, 0) + 1
...
def create_test_folder_structure(base_dir: Path) -> Path:
    ...
    for i in range(3):
        ...
        aux_file.write_text('<PAMDataset><Metadata><MDI key="AREA_OR_POINT">Area</MDI></Metadata></PAMDataset>')
    ...
    LOGGER.info(f"Created test folder structure with {len(test_files)} files in {test_dir}")
    return test_dir