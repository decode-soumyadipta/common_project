...
class VisualizationCoordinator:
    ...
    def apply_visual_settings(self, log_to_panel: bool = True) -> None:
        ...
        if log_to_panel:
            c.panel.log(f"Applied imagery settings brightness={brightness:.2f}, contrast={contrast:.2f}")
            c._logger.info("Applied imagery settings brightness=%.2f contrast=%.2f", brightness, contrast)
            return
        c._logger.debug("Live imagery settings brightness=%.2f contrast=%.2f", brightness, contrast)
    ...
    def apply_dem_settings(self, _checked: bool | None = None, log_to_panel: bool = True) -> None:
        ...
        if log_to_panel:
            c.panel.log("Applied DEM settings " f"exaggeration={exaggeration:.2f}, hillshade={hillshade_strength:.2f}")
            c._logger.info(
