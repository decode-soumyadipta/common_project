...
_BASE_SRC = Path(__file__).resolve().parent
ICON_DIR = _BASE_SRC / "icons"

if not ICON_DIR.exists():
    # Fallback for modular structure
    ICON_DIR = _BASE_SRC / "client_frontend" / "icons"
...
ICON_MANIFEST: dict[str, str] = {
    ...
    "split_view": "mActionSplitFeatures.svg",
    "fullscreen": "mActionNewMap.svg",
    ...
    "data_source_manager": "mActionDataSourceManager.svg",
}
...
class IconRegistry:
    ...
    def _make_fly_through_icon(size: int, color: Optional[str]) -> QIcon:
        ...
        trail_color = QColor(color or "#1e88e5")
        body_color = QColor(color or "#111111")

        painter.setBrush(trail_color)
        trail = QPainterPath()
        trail.moveTo(size * 0.08, size * 0.72)
        trail.cubicTo(size * 0.26, size * 0.64, size * 0.40, size * 0.54, size * 0.54, size * 0.44)
        trail.cubicTo(size * 0.66, size * 0.36, size * 0.78, size * 0.30, size * 0.92, size * 0.22)
        painter.drawPath(trail)

        painter.setBrush(body_color)
        plane = QPainterPath()
        plane.moveTo(size * 0.18, size * 0.56)
        plane.lineTo(size * 0.56, size * 0.44)
        plane.lineTo(size * 0.78, size * 0.18)
        plane.lineTo(size * 0.74, size * 0.48)
        plane.lineTo(size * 0.90, size * 0.66)
        plane.lineTo(size * 0.68, size * 0.62)
        plane.lineTo(size * 0.58, size * 0.84)
        plane.lineTo(size * 0.48, size * 0.58)
        plane.lineTo(size * 0.28, size * 0.68)
        plane.closeSubpath()
        painter.drawPath(plane)

        painter.setBrush(QColor("#f4f4f4") if not color else body_color.lighter(145))
        wing = QPainterPath()
        wing.moveTo(size * 0.36, size * 0.50)
        wing.lineTo(size * 0.56, size * 0.42)
        wing.lineTo(size * 0.44, size * 0.68)
        wing.closeSubpath()
