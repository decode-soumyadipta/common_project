...
from src_new.clients.desktop_search.logging_setup import (
    configure_desktop_logging,
)
from src_new.clients.desktop_search.qt_compat import ensure_desktop_qt_runtime
...
def run(
    ...
    # Set up a clean cache directory to avoid permission issues when running multiple instances
    # (e.g. Ingest UI and Search UI simultaneously).
    cache_dir = Path(tempfile.gettempdir()) / f"offline_gis_cache_{app_mode.value}"
    cache_dir.mkdir(parents=True, exist_ok=True)
    os.environ["QTWEBENGINE_CACHE_DIR"] = str(cache_dir)
    os.environ["QTWEBENGINE_USER_DATA_DIR"] = str(cache_dir) # Some versions use this
    ...
    required_flags = [
        ...
        "--disable-gpu-shader-disk-cache",  # Extra precaution against GPU cache lock
