...
function Ensure-CondaQtWebEngine {
    ...
    if (Try-CondaInstall -EnvName $EnvName -Packages @("pyqt", "pyqtwebengine")) {
        Write-Host "Installed Qt runtime via conda packages: pyqt + pyqtwebengine" -ForegroundColor Green
        return
    }

    Write-Host "Conda pyqt packages unavailable. Falling back to pip Qt packages inside conda env..." -ForegroundColor Yellow
    Run-Step -Message "Installing pip Qt fallback packages in conda env" -Action {
        conda run -n $EnvName python -m pip install --upgrade "PyQt5>=5.15,<6.0" "PyQtWebEngine>=5.15,<6.0" "qtpy>=2.4,<3.0"
    }

    $global:LASTEXITCODE = 0
    conda run -n $EnvName python -c "from qtpy.QtWebEngineWidgets import QWebEngineView; print('QtWebEngine OK')"
    if ($global:LASTEXITCODE -eq 0) {
        Write-Host "Installed Qt runtime via pip fallback: PyQt5 + PyQtWebEngine + qtpy" -ForegroundColor Green
        return
    }

    throw "Unable to install a working Qt WebEngine runtime in conda environment '$EnvName'."
}
...
if ($selectedMode -eq "conda") {
    ...
    $qtCheckCode = "from qtpy.QtWebEngineWidgets import QWebEngineView; print('QtWebEngine OK')"
...
Run-Step -Message "Verifying Qt WebEngine import in venv" -Action {
    & $venvPython -c "from qtpy.QtWebEngineWidgets import QWebEngineView; print('QtWebEngine OK')"
}
