...
def test_titiler_server():
    ...
    print(f"  - Bounds: west={actual_bounds['west']:.6f}, south={actual_bounds['south']:.6f}")
    print(f"           east={actual_bounds['east']:.6f}, north={actual_bounds['north']:.6f}")
    ...
    for zoom in [10, 11, 12, 13, 14]:
        ...
        tile_url = f"{base_url}/cog/tiles/WebMercatorQuad/{zoom}/{x}/{y}.png?url={file_url}"
        try:
            tile_response = requests.get(tile_url, timeout=10)
            if tile_response.status_code == 200:
                print(f"    ✓ Tile z={zoom}/x={x}/y={y} SUCCESS ({len(tile_response.content)} bytes)")
            elif tile_response.status_code == 404:
                print(f"    ✗ Tile z={zoom}/x={x}/y={y} NOT FOUND (404)")
            else:
                print(f"    ✗ Tile z={zoom}/x={x}/y={y} ERROR ({tile_response.status_code})")
        except requests.exceptions.RequestException as e:
    ...
    for zoom in [11, 12]:
        ...
        try:
            tile_response = requests.get(tile_url, timeout=10)
            status = "✓ SUCCESS" if tile_response.status_code == 200 else f"✗ {tile_response.status_code}"
    ...
    for dx in [-1, 0, 1]:
        for dy in [-1, 0, 1]:
            ...
            try:
                tile_response = requests.get(tile_url, timeout=10)
                if tile_response.status_code == 200:
                    print(f"    ✓ Tile 12/{test_x}/{test_y} SUCCESS ({len(tile_response.content)} bytes)")
                elif tile_response.status_code == 404:
                    print(f"    ✗ Tile 12/{test_x}/{test_y} NOT FOUND (404)")
                else:
                    print(f"    ✗ Tile 12/{test_x}/{test_y} ERROR ({tile_response.status_code})")
            except requests.exceptions.RequestException as e:
