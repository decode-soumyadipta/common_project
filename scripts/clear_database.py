...
def find_all_database_files():
    ...
    locations_to_check = [
        ...
        Path.home() / "AppData" / "Local" / "offline_gis" if platform.system() == "Windows" else Path("/tmp"),
    ]
...
def check_running_processes():
    ...
    try:
        ...
        process_names = [
            "python", "python3", "pythonw", "pythonw.exe",
            "offline_gis", "desktop_client", "titiler"
        ]
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                ...
                if any(name in proc_name for name in process_names):
                    if any(keyword in cmdline for keyword in ['offline_gis', 'desktop_client', 'run_desktop']):
    ...
    try:
        ...
        for proc_info in processes:
            try:
                ...
                if force:
                    proc.kill()
                    print(f"💀 Force killed PID {proc_info['pid']}: {proc_info['name']}")
                else:
                    proc.terminate()
...
def clear_database():
    ...
    for file_path in files_to_delete:
        try:
            ...
            total_size += size
            file_type = "DB" if file_path.suffix == ".db" else file_path.suffix.upper().lstrip(".-")
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt < max_attempts - 1:
                        print(f"⏳ File locked, retrying... ({attempt + 1}/{max_attempts})")
                        time.sleep(1)
                    else:
                        raise e
